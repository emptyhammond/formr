...
class Kohana_Formr_Render extends Kohana_Formr
{
	...
	protected static function select($relation, $multi = false, $object, $options)
	{
		...
		if (isset($options['sources'][$relation['relation_name']])) // An array source is specified
		{
			...
			$attributes['name'] = $name = ($multi) ? $name.'[]' : $relation['relation_name'];
		}
		...
		else
		{
			...
			else
			{
				...
				{
					$selected = $object->{$relation['relation_name']}->loaded() ? $object->{$relation['relation_name']}->pk() : (isset($options['default'][$relation['relation_name']]) ? $options['default'][$relation['relation_name']] : null);
				}
			}
		}
