...
class Kohana_Formr_Render extends Kohana_Formr
{
	...
	protected static function date($column, $object, $options)
	{
		...
		return View::factory(Kohana::$config->load('formr.render').'/date')->bind('disabled',$disabled)->bind('value',$value)->bind('column',$column)->bind('object',$object)->bind('options',$options)->render();
	}
	...
	protected static function select($relation, $multi = false, $object, $options)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					$selected = $object->{$relation['relation_name']}->loaded() ? $object->{$relation['relation_name']}->pk() : (isset($options['values'][$relation['relation_name']]) ? $options['values'][$relation['relation_name']] : (isset($options['default'][$relation['relation_name']]) ? $options['default'][$relation['relation_name']] : 0));
				}
			}
		}
